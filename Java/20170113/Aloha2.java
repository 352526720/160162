...
class Fileman{
    ...
    public void writeData(String[] data)
    {
        ...
        try {
            File outYang = new File("data.txt");
            pwrite = new PrintWriter(new BufferedWriter(new FileWriter(outYang, true)));

            for (int i = 0; i < data.length; i++ ) {
                 String msgYang = data[i];
                 if (msgYang != null) {
                     pwrite.println(msgYang);
                    System.out.println(msgYang);
                 }
            }
        } catch( IOException e) {
